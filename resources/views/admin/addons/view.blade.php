                ...
                <div class="box-body">
                            ...
                            @foreach($nests as $nest)
                                @foreach($nest->eggs as $egg)
                                    @foreach($egg->addonCategories as $category)
                                        <option value="{{ $category->id }}" data-egg-id="{{ $egg->id }}" {{ old('category_id', $addon->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }} ({{ $egg->name }})</option>
                                    @endforeach
                                @endforeach
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <div class="checkbox checkbox-primary no-margin-bottom">
                            <input id="pReinstallServer" name="reinstall_server" type="checkbox" value="1" {{ old('reinstall_server', $addon->reinstall_server) ? 'checked' : '' }} />
